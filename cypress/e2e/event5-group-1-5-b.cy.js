/**
 * Test Event 5: Random Generate 2
 * 
 * Event ini memiliki grup dengan random 2-5 anggota.
 * Total members: 222, jumlah grup bervariasi (44-111 grup)
 */

describe('Event 5: Lottery Drawing - Random Generate 2', () => {
  beforeEach(() => {
    cy.login()
  })

  it('should draw ALL groups for Event 5', () => {
    cy.contains('.card-title', 'Event Test - Grup 1-5 Orang (B)').click()
      ...
      cy.get('body').then($body => {
        if ($body.find('.btn-draw').length > 0) {
          ...
          cy.get('#random-stall-number').invoke('text').then(number => {
            ...
            groupsDrawn++
            cy.log(`Event 5 - Draw ${groupsDrawn}: Number ${number}`)
          })
          ...
          cy.log(`✓ All ${groupsDrawn} groups drawn for Event 5!`)
        }
      })
    }
    ...
    cy.then(() => {
      ...
      cy.log(`=== Event 5 Final Statistics ===`)
    ...
    cy.screenshot('event5-all-groups-drawn')
  })
  ...
  it('should verify number distribution', () => {
    ...
    cy.then(() => {
      ...
      cy.log('=== Event 5 Number Distribution ===')

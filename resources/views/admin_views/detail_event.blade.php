                ...
                <div class="tab-pane fade document-link-wrapper bg-white p-2" data-cy="wrapper-document-link"
                    ...
                    @if ($count > 0)
                        <div class="draw-section mb-4 p-4 position-relative overflow-hidden"
                            ...
                            @php
                                // Hitung peserta yang belum bayar
                                $unpaidGroups = collect($event->groups->toArray())
                                    ->where('status', 'unpaid')
                                    ->where('raffle_status', 'not_yet');
                                $unpaidCount = $unpaidGroups->count();
                            @endphp

                            {{-- Alert untuk peserta belum bayar --}}
                            @if ($unpaidCount > 0)
                                <div class="alert alert-danger d-flex align-items-center mb-4 position-relative"
                                    role="alert"
                                    style="background: #dc3545; 
                                            border: 2px solid #b02a37; 
                                            border-radius: 15px; 
                                            box-shadow: 0 4px 15px rgba(220, 53, 69, 0.4);">
                                    <i class="ri-alert-line me-3" style="font-size: 2rem; color: #ffffff;"></i>
                                    <div>
                                        <h6 class="alert-heading mb-1 fw-bold" style="color: #ffffff;">
                                            <i class=""></i>
                                            Perhatian: Ada Peserta Belum Bayar
                                        </h6>
                                        <p class="mb-0" style="color: #ffffff; font-size: 0.95rem;">
                                            Terdapat <strong>{{ $unpaidCount }} grup peserta</strong> dengan status
                                            <strong>Belum Bayar</strong>
                                            yang tidak akan diikutkan dalam pengundian.
                                            Silakan ubah status pembayaran terlebih dahulu jika ingin<strong> mengikutkan
                                                mereka.</strong>
                                        </p>
                                    </div>
                                </div>
                                    ...
                                    <div class="col-md-4">
                                        <div class="d-flex flex-column">
                                            ...
                                            <span class="fw-bold fs-5"
                                                style="color: #0d5c3a;">{{ $groups_not_yet_drawn[0]['name'] }}</span>
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="d-flex flex-column">
                                            ...
                                            <div class="d-flex align-items-center gap-2">
                                                <span class="fw-bold fs-5" id="phoneNumber"
                                                    style="color: #0d5c3a;">{{ $groups_not_yet_drawn[0]['phone_num'] ? str_repeat('•', strlen($groups_not_yet_drawn[0]['phone_num'])) : '-' }}</span>
                                                <span class="fw-bold fs-5 d-none" id="phoneNumberReal"
                                                    style="color: #0d5c3a;">{{ $groups_not_yet_drawn[0]['phone_num'] ?? '-' }}</span>
                                                <button type="button"
                                    ...
                                    <div class="col-md-4">
                                        <div class="d-flex flex-column">
                                            ...
                                            <span class="badge d-inline-flex align-items-center"
                                                ...
                                                <span>{{ $groups_not_yet_drawn[0]['total_member'] }} Orang</span>
                                            </span>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="text-center position-relative">
                                <button data-participant-group-id="{{ $groups_not_yet_drawn[0]['id'] }}"
                                ...
                                <div class="left-column col-1 d-flex gap-2" style="position: relative; z-index: 2;">
                                    <div class="box-wrapper d-flex flex-column gap-2" style="min-width: 70px;">
                                        @foreach ($leftColumnParticipant as $data)
                                            <div @class([
                                                'stall-box',
                                                'border border-3 rounded d-flex justify-content-center align-items-center fw-bold',
                                                'bg-danger text-white' => $data['isBooked'],
                                                'bg-warning text-dark' => !$data['isBooked'],
                                            ])
                                    ...
                                    <div class="name-wrapper d-flex flex-column gap-2">
                                        @foreach ($leftColumnParticipant as $data)
                                            @if ($data['participant_name'])
                                                <div @class([
                                                    'participant-name-card border border-3 rounded d-flex align-items-center px-3 fw-bold',
                                                    'bg-danger text-white' => $data['isBooked'],
                                                    'bg-warning text-dark' => !$data['isBooked'],
                                                ])
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="d-flex align-items-center justify-content-center gap-2">
                                        ...
                                        <span class="badge"
                                            ...
                                            <i
                                                class="ri-group-line me-1"></i>{{ $groups_not_yet_drawn[0]['total_member'] }}
                                            orang
                                        </span>
                                    </div>
                                </div>
        ...
        $(document).on("click", ".btn-draw", function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    // Hanya proses UPPER/UNDER jika bukan scattered DAN total_member > 1
                    if (!response.data.isScattered && response.data.total_member > 1) {
                        ...
                        // Untuk scattered ATAU total_member === 1, tampilkan single button
                        $(".confirm-wrapper-single").removeClass('d-none');
                        $(".confirm-wrapper-multiple").addClass('d-none');
                    }
